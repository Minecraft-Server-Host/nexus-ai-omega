...
// FIX: handleGlobalBanButton fehlte — wird von buttonHandler.ts benötigt
export async function handleGlobalBanButton(interaction: import('discord.js').ButtonInteraction): Promise<void> {
  const [, action, userId] = interaction.customId.split('_');
  if (action === 'appeal' || action === 'evidence' || action === 'unban') {
    await interaction.reply({ content: `🔍 Aktion: **${action}** für User \`${userId}\` — bitte im Nexus-Team-Kanal fortfahren.`, ephemeral: true });
  } else if (action === 'prev' || action === 'next') {
    // Pagination — einfach bestätigen
