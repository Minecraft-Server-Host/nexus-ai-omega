...
async function boot(): Promise<void> {
  ...
  // Register bot client with panel routes for live stats
  try {
    const { setBotClient } = await import('../api/server.js');
    setBotClient(client);
    logger.info('🔗 Bot client registered with Team Panel');
  } catch { /* non-fatal if panel not loaded */ }
}
